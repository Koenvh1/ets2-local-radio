...
function refresh(data) {
    ...
    */
    ...
    if (!(data.TruckValues.Positioning.HeadPositionInWorldSpace.X == 0.0 && data.TruckValues.Positioning.HeadPositionInWorldSpace.Y == 0.0 && data.TruckValues.Positioning.HeadPositionInWorldSpace.Z == 0.0) || g_show_all) {
        for (var i = 0; i < cities.length; i++) {
            //Fix uppercase issues (*cough* SCS):
            cities[i]["country"] = cities[i]["country"].toLowerCase();
            //Calculate distance
            var distance = Math.sqrt(Math.pow((data.TruckValues.Positioning.HeadPositionInWorldSpace.X - cities[i]["x"]), 2) + Math.pow((data.TruckValues.Positioning.HeadPositionInWorldSpace.Z - cities[i]["z"]), 2));
